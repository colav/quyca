def test_it_can_plot_most_used_title_words_by_institution(client, random_institution_id):
    response = client.get(
...
def test_it_can_plot_most_used_title_words_by_faculty(client, random_faculty_id):
    response = client.get(f"/app/affiliation/faculty/{random_faculty_id}/research/products?plot=most_used_title_words")
    assert response.status_code == 200


def test_it_can_plot_most_used_title_words_by_department(client, random_department_id):
    response = client.get(
...
def test_it_can_plot_most_used_title_words_by_group(client, random_group_id):
